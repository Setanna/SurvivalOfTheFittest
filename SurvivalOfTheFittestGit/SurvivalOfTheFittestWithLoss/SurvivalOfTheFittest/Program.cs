...
namespace SurvivalOfTheFittestWithLoss
{
    ...
    class Program
    {
        static class Environment
        {
            ...
            public static List<Creature> Carnivores = CreateCreatures(20, "Carnivore");
            public static List<Creature> Herbivores = CreateCreatures(200, "Herbivore");
            public static int DesiredCarnivores = 200;
            public static int DesiredHerbivores = 2000;
        }
        public class Stats
        {
        ...
        public class Creature
        {
            ...
            public int loss { get; set; } // How well the creatures does.
        }
        ...
        static void Main(string[] args)
        {
            ...
            List<double> herbivoreLoss = new List<double>();
            ...
            List<double> carnivoreLoss = new List<double>();
            ...
            while (Carnivores.Count != 0 && Herbivores.Count != 0)
            {
                ...
                if (Herbivores.Count > 0)
                {
                    ...
                    herbivoreLoss.Add(Herbivores.Sum(c => c.loss) / Herbivores.Count);
                }
                else
                {
                ...
                if (Carnivores.Count > 0)
                {
                    ...
                    carnivoreLoss.Add(Carnivores.Sum(c => c.loss) / Carnivores.Count);
                }
                else
                {
            ...
            CreateScottPlot(day, herbivoreLoss, "Day", "Herbivore Loss", "HerbivoreLoss");
            ...
            CreateScottPlot(day, carnivoreLoss, "Day", "Carnivore Loss", "CarnivoreLoss");
        ...
        static void CreateScottPlot(List<double> x, List<double> y, string xLabel, string yLabel, string graphName)
        {
            ...
            plt.SaveFig(@"C:\Users\joenh\OneDrive\Skrivebord\Education\School\Hovedforløb 4\Machine Learning\SurvivalOfTheFittestWithLoss\Graphs\" + graphName + ".png");
        ...
        static void Breed(List<Creature> creatures)
        {
            ...
            foreach (Creature male in males.OrderBy(c => c.loss).ToList())
            {
                // Foreach female
                foreach (Creature female in females.OrderBy(c => c.loss).ToList())
                {
                    // If the female is not carrying a child
                    if (female.children == null && female.hunger < 50 && rnd.Next(0, 100) < male.desirability)
                    {
        ...
        static List<Creature> Child(Creature mother, Creature father)
        {
            ...
            for (int i = 0; i < litter; i++)
            {
                ...
                child.age = 0;
                child.maxAge = InherentGene(mother.maxAge, father.maxAge);
                child.desirability = InherentGene(mother.desirability, father.desirability);
                child.loss = Loss(child.litter, child.gestation, child.maxAge, child.hungerType);
        ...
        // Calculate Loss
        static int Loss(int litter, int gestation, int maxAge, string hungerType)
        {
            int Loss = 0;

            if (hungerType == "Herbivore")
            {
                Loss += Math.Abs(Environment.DesiredHerbivores - Environment.Herbivores.Count());
                Loss += Environment.Herbivores.Count > Environment.DesiredHerbivores ? -litter : +litter;
                Loss += Environment.Herbivores.Count > Environment.DesiredHerbivores ? -gestation : +gestation;
                Loss += Environment.Herbivores.Count > Environment.DesiredHerbivores ? -maxAge : +maxAge;
            }
            else
            {
                Loss += Math.Abs(Environment.DesiredCarnivores - Environment.Carnivores.Count());
                Loss += Environment.Carnivores.Count > Environment.DesiredCarnivores ? -gestation : +gestation;
                Loss += Environment.Carnivores.Count > Environment.DesiredCarnivores ? -maxAge : +maxAge;
            }
        ...
        static List<Creature> CreateCreatures(int amount, string hungerType)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                if (i % 2 == 0)
                {
                    ...
                    // Set the childs attributes
                    creature.hunger = rnd.Next(1, 10) * 10;
                    creature.hungerType = hungerType;
                    creature.gender = true;
                    creature.gestation = rnd.Next(5, 10);
                    creature.litter = hungerType == "Herbivore" ? rnd.Next(1, 3) : 1;
                    creature.age = 0;
                    creature.maxAge = rnd.Next(10, 30);
                    creature.desirability = rnd.Next(10, 30);
                    creature.loss = hungerType == "Herbivore" ? 4500 - creature.gestation - creature.litter : 450 - creature.gestation;
